...
SELECT LEFT([REF],6) as PNR
  ...
  Where [Type de la transaction] = 'Post Autorisation'
...
--   into [Tickets].[dbo].[Réconciliation]
  FROM #tempTicket a
  inner join  #tempALL b on a.[PNR]=b.[PNR] /*and convert(datetime,[PAYDATE],103)=convert(datetime,[issue date] ,103)*/
) k
  
  Select * from  [Tickets].[dbo].[Réconciliation]
  Where [Ecart] <>0
