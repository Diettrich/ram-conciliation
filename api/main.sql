...
SELECT g.[PNR]
	...
	Into #tempAPP
      ...
      ,convert(date,concat(left([Timestamp],4),'/',SUBSTRING([Timestamp],5,2),'/',SUBSTRING([Timestamp],7,2))) as [Payment Date]
	  ,b.[Country] as [Marché]
	  ,case When ([OfficeID] like 'AMMAT%' or [OfficeID] like 'TUNAT%') then sum(convert(float,[Value]))/1000
	  When ([OfficeID] like 'DKRAT%' or [OfficeID] like 'ABJAT%' or [OfficeID] like 'DLAAT%') then sum(convert(float,[Value]))
	  Else sum(convert(float,[Value]))/100 End as [Montant]
      ,[Currency]
	  ,[Canal]='APP'
...
SELECT LEFT([REF],6) as PNR
      ...
      ,b.[Country]
	  ,Case when [ORIGINAL_CUR]= ''  then sum([TOTAL])
	  Else sum( [ORIGINAL_AMOUNT] ) End as [Amount] 
	  ,Case when [ORIGINAL_CUR]  = '' then [CUR]
	  Else [ORIGINAL_CUR] End as [Devise]
	  ,[Canal]='OGONE'
  ...
  from (
  SELECT 
	  case when len(REPLACE(REPLACE([Numéro de la commande],'-',''), ' ', ''))=6 then REPLACE(REPLACE([Numéro de la commande],'-',''), ' ', '')
	  when len( LEFT(REPLACE(REPLACE([Numéro de la commande],'-',''), ' ', ''),CHARINDEX('_', REPLACE(REPLACE([Numéro de la commande],'-',''), ' ', ''))))=7 then  
	  LEFT(REPLACE(REPLACE([Numéro de la commande],'-',''), ' ', ''),CHARINDEX('_', REPLACE(REPLACE([Numéro de la commande],'-',''), ' ', ''))-1)
	  else RIGHT(REPLACE([Numéro de la commande], ' ', ''), 6)
	  end as PNR
      ,convert(date,LEFT([Date de la transaction],10)) as [Date de la transaction]
	  ,iif ([Devise]='MAD','Morocco',iif([Devise]='CAD','CANADA',iif([Devise]='EUR','France',iif([Devise]='USD','United States',iif([Devise]='GBP','United Kingdom',iif([Devise]='CHF','Switzerland','')))))) as [Marché]  
      ,[Montant]
      ,[Devise]
	  ,[Canal]='CMI'
  ...
  and [Nom de la société] in ('RAM AGENT PAY'  ,'RAM CALL CENTER'  ,'RAM CALL CENTER CAD', 'RAM CALL CENTER EUR' 
  ,'RAM CALL CENTER USD' , 'RAM CALL CENTER GBP' ,'RAM CALL CENTER CHF' , 'RAM eRetail','RAM INTERNET ECOM', 'RAM INTERNET UPGRADE')
...
SELECT [Réf# commande marchand] as [PNR]
	  ...
	  ,iif([Marchand]='Royal Air Maroc','France',iif([Marchand]='Royal Air Maroc DE', 'Allemagne', iif([Marchand]='Royal Air Maroc UK','UK',''))) as [Marché]
      ,[Montant]
FROM [Thunes ].[dbo].[thunes$]
Where [Type]='Recrédit') a
...
Select [N° PNR] as [PNR]
	...
	SELECT [N° PNR]
	  , convert(date, REPLACE(SUBSTRING([Payment Date],LEN(LEFT([Payment Date],CHARINDEX(' ', [Payment Date])+1)),LEN([Payment Date]) - LEN(LEFT([Payment Date],CHARINDEX(' ', [Payment Date]))) - LEN(RIGHT([Payment Date],CHARINDEX(' ', (REVERSE([Payment Date])))))),'-','/')) AS [Payment Date]
	--   ,[Marché]='Morocco'
      ,[Amount]
    --   ,[Devise]='MAD'
	--   ,[Canal]='BINGA'
	 ...
	 Where [Status] in ('PAID', 'ISSUED')) as temp2
...
-- Select * From #tempBINGA
 ...
 Select [PNR]
  ...
  where [Banque ou canal de paiement]<>'') a
  Group by [PNR]
       ,[Date de la facture]
	   ,[Marché]
	   ,[Devise]
...
/****** Retraitement ******/
    ...
    select [PNR], [PAYDATE], [Country] COLLATE SQL_Latin1_General_CP1_CI_AS [Country], [Amount], [Devise] COLLATE SQL_Latin1_General_CP1_CI_AS [Devise], [Canal] Into #tempALL from #tempBINGA
    UNION all
    select [PNR], [PAYDATE], [Country] COLLATE SQL_Latin1_General_CP1_CI_AS, [Amount], [Devise] COLLATE SQL_Latin1_General_CP1_CI_AS, [Canal] from #tempOGONE
    UNION all
    select [PNR], [PAYDATE], [Country] COLLATE SQL_Latin1_General_CP1_CI_AS, [Amount], [Devise] COLLATE SQL_Latin1_General_CP1_CI_AS, [Canal] from #tempCMI
    UNION all
    select [PNR], [PAYDATE], [Country] COLLATE SQL_Latin1_General_CP1_CI_AS, [Amount], [Devise] COLLATE SQL_Latin1_General_CP1_CI_AS, [Canal] from #tempAPP
    UNION all
    select [PNR], [PAYDATE], [Country] COLLATE SQL_Latin1_General_CP1_CI_AS, [Amount], [Devise] COLLATE SQL_Latin1_General_CP1_CI_AS, [Canal] from #tempThunes
    UNION all
    select [PNR], [PAYDATE], [Country] COLLATE SQL_Latin1_General_CP1_CI_AS, [Amount], [Devise] COLLATE SQL_Latin1_General_CP1_CI_AS, [Canal] from #tempFatourati
...
select * from (SELECT [issue date] as [PAYDATE]
      ,a.[PNR]
	  ,[Entité]
      ,[Total] as [Amount_Altea]
	  ,a.[Devise] as [Devise_Altea]
